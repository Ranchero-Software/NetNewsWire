//
//  FeedlyMainThreadOperationQueue.swift
//  RSCore
//
...
// Legacy code. Used only by Feedly.
// TODO: Replace with MainThreadOperationQueue.

public protocol FeedlyMainThreadOperationDelegate: AnyObject {
	func operationDidComplete(_ operation: FeedlyMainThreadOperation)
	func cancelOperation(_ operation: FeedlyMainThreadOperation)
	func make(_ childOperation: FeedlyMainThreadOperation, dependOn parentOperation: FeedlyMainThreadOperation)
}

/// Manage a queue of FeedlyMainThreadOperation tasks.
///
...
@MainActor public final class FeedlyMainThreadOperationQueue {

	/// Use the shared queue when you don’t need to create a separate queue.
	public static let shared: FeedlyMainThreadOperationQueue = {
		FeedlyMainThreadOperationQueue()
	}()

	private var operations = [Int: FeedlyMainThreadOperation]()
	private var pendingOperationIDs = [Int]()
	private var currentOperationID: Int?
	private static var incrementingID = 0
	private var isSuspended = false
	private let dependencies = FeedlyMainThreadOperationDependencies()
	...
	public func add(_ operation: FeedlyMainThreadOperation) {
	...
	public func addOperations(_ operations: [FeedlyMainThreadOperation]) {
	...
	public func make(_ childOperation: FeedlyMainThreadOperation, dependOn parentOperation: FeedlyMainThreadOperation) {
		precondition(Thread.isMainThread)
	...
	public func cancelOperations(_ operations: [FeedlyMainThreadOperation]) {
		precondition(Thread.isMainThread)
...
extension FeedlyMainThreadOperationQueue: FeedlyMainThreadOperationDelegate {

	public func operationDidComplete(_ operation: FeedlyMainThreadOperation) {
		precondition(Thread.isMainThread)
		operationDidFinish(operation)
	}

	public func cancelOperation(_ operation: FeedlyMainThreadOperation) {
		cancelOperations([operation])
	}
}

private extension FeedlyMainThreadOperationQueue {

	var pendingOperations: [FeedlyMainThreadOperation] {
		return pendingOperationIDs.compactMap { (operationID) -> FeedlyMainThreadOperation? in
	...
	var currentOperation: FeedlyMainThreadOperation? {
	...
	func pendingAndCurrentOperations(named name: String) -> [FeedlyMainThreadOperation]? {
	...
	func operationDidFinish(_ operation: FeedlyMainThreadOperation) {
	...
	func popNextAvailableOperation() -> FeedlyMainThreadOperation? {
	...
	func operationIsAvailable(_ operation: FeedlyMainThreadOperation) -> Bool {
	...
	func ensureOperationID(_ operation: FeedlyMainThreadOperation) -> Int {
	...
	func removeFromStorage(_ operation: FeedlyMainThreadOperation) {
		guard let operationID = operation.id else {
			assertionFailure("Expected operation.id, got nil.")
			return
		}
	...
	func callCompletionBlockForOperations(_ operations: [FeedlyMainThreadOperation]) {
		for operation in operations {
			callCompletionBlock(for: operation)
		}
	}

	func callCompletionBlock(for operation: FeedlyMainThreadOperation) {
...
private final class FeedlyMainThreadOperationDependencies {
...
private extension FeedlyMainThreadOperationDependencies {
