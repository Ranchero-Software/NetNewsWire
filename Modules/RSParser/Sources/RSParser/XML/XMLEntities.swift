...
// Entity decoding for the XML and HTML scanners.
//
// "Predefined XML entities" = the five entities XML 1.0 requires every parser
// to recognize without a DTD: &amp; &lt; &gt; &quot; &apos;.
//
// Two modes, chosen per caller to match the corresponding libxml2 behavior
// so feed output is byte-identical to the legacy parsers:
//
// - `.xmlStrict`: expand predefined XML entities and numeric entities only
//   (libxml2 XML parser without a DTD). Unknown named entities like `&mdash;`
//   pass through literally.
//
// - `.html`: expand predefined XML entities, numeric entities, and HTML named
//   entities (libxml2 HTML parser).
//
// CDATA content in XML documents should NOT go through this function at all —
// libxml2 passes CDATA contents through raw, and we match that.
//
...
enum XMLEntities {
	...
	enum Mode {
		/// HTML mode: expand predefined XML entities, numeric entities, and HTML
		/// named entities (`&mdash;`, `&nbsp;`, etc.).
		case html
		/// XML mode (libxml2-compatible without a DTD): expand predefined XML
		/// entities and numeric entities only. Unknown named entities pass through
		/// literally.
		case xmlStrict
	}
	...
	static func decode(bytes: [UInt8], at: Int, mode: Mode) -> DecodedEntity {
		...
		// Predefined XML entities are expanded in every mode.
		if let predefined = xmlPredefinedEntity(name: name) {
			return DecodedEntity(bytes: predefined, nextIndex: semicolonIndex + 1)
		}

		// HTML named entities are only expanded in .html mode (matches libxml2
		// HTML parser behavior; libxml2 XML parser without a DTD leaves them literal).
		if mode == .html, let html = htmlNamedEntities[asciiString(from: name)] {
