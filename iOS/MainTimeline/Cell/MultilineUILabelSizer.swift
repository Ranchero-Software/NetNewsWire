...
// Get the height of a UILabel given a string, font, and width.
...
struct TextFieldSizeInfo {
	...
	let numberOfLinesUsed: Int // A two-line label may only use one line, for instance. This would equal 1, then.
}
...
@MainActor final class MultilineUILabelSizer {
	...
	static func size(for string: String, font: UIFont, numberOfLines: Int, width: Int) -> TextFieldSizeInfo {
		sizer(numberOfLines: numberOfLines, font: font).sizeInfo(for: string, width: width)
	}
...
private extension MultilineUILabelSizer {
	...
	func sizeInfo(for string: String, width: Int) -> TextFieldSizeInfo {
		let labelHeight = height(for: string, width: width)
		let numberOfLinesUsed = numberOfLines(for: labelHeight)
		let size = CGSize(width: width, height: labelHeight)
		return TextFieldSizeInfo(size: size, numberOfLinesUsed: numberOfLinesUsed)
	}
	...
	func numberOfLines(for height: Int) -> Int {
		...
		return Int(round(CGFloat(height) / averageHeight))
	}

	func heightIsProbablySingleLineHeight(_ height: Int) -> Bool {
		heightIsProbablyEqualToEstimate(height, singleLineHeightEstimate)
	}

	func heightIsProbablyDoubleLineHeight(_ height: Int) -> Bool {
		heightIsProbablyEqualToEstimate(height, doubleLineHeightEstimate)
	}
