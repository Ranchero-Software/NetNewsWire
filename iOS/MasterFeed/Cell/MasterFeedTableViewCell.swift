	...
	func setDisclosure(isExpanded: Bool, animated: Bool) {
		...
		UIView.animate(withDuration: duration) {
			if self.isDisclosureExpanded {
				self.disclosureButton?.accessibilityLabel = NSLocalizedString("COLLAPSE_FOLDER", comment: "Collapse Folder")
				self.disclosureButton?.imageView?.transform = CGAffineTransform(rotationAngle: 1.570796)
			} else {
				self.disclosureButton?.accessibilityLabel = NSLocalizedString("EXPAND_FOLDER", comment: "Expand Folder") 
